/**
 * Compact, dependency-free MD5 over bytes → lowercase hex.
 *
 * Produces the identical digest to Node's crypto.createHash('md5'), so the
 * browser importer hashes tile images to the same keys the tile table (built
 * with Node) uses. Standard RFC 1321 algorithm.
 */

function toWords(bytes: Uint8Array): { words: number[]; bits: number } {
  const len = bytes.length;
  const words: number[] = [];
  for (let i = 0; i < len; i++) words[i >> 2] |= bytes[i] << ((i % 4) * 8);
  return { words, bits: len * 8 };
}

function add32(a: number, b: number): number { return (a + b) & 0xffffffff; }
function rol(x: number, c: number): number { return (x << c) | (x >>> (32 - c)); }
...
export function md5hex(bytes: Uint8Array): string {
  const { words, bits } = toWords(bytes);
  // padding
  words[bits >> 5] |= 0x80 << (bits % 32);
